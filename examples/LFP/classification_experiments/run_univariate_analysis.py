...
DEFAULT_UNIVARIATE_N_PERMUTATIONS = 1000
DEFAULT_UNIVARIATE_RANDOM_STATE = 42
DEFAULT_UNIVARIATE_TOP_K = 25
DEFAULT_UNIVARIATE_PERM_PROGRESS = 25
DEFAULT_VIS_TOP_K = 10

METRICS = [
    ('univ_anova_f', 'ANOVA F-score'),
    ('univ_mutual_info', 'Mutual Information'),
    ('univ_mann_whitney_u', 'Mann–Whitney U'),
    ('univ_brunner_munzel_stat', 'Brunner–Munzel Statistic'),
    ('univ_permutation_mean_diff', 'Permutation Mean Difference'),
]
...
def permutation_mean_difference(feature_values, labels, n_permutations=1000, random_state=42):
    g0 = feature_values[labels == 0]
    g1 = feature_values[labels == 1]
    mask0 = np.isfinite(g0)
    mask1 = np.isfinite(g1)
    g0 = g0[mask0]
    g1 = g1[mask1]
    ...
    observed = np.abs(np.nanmean(g1) - np.nanmean(g0))
    ...
    for _ in range(n_permutations):
        ...
        perm_g0 = combined[:len(g0)]
        perm_g1 = combined[len(g0):]
        perm_stat = np.abs(np.nanmean(perm_g1) - np.nanmean(perm_g0))
...
def compute_permutation_scores(X, y, n_permutations=1000, random_state=42,
    ...
    for idx in range(total_features):
        stat, p_val = permutation_mean_difference(
            X[:, idx], y, n_permutations=n_permutations,
            random_state=random_state + idx
        )
        stats_arr[idx] = stat
        p_vals[idx] = p_val
        if progress_interval and (idx + 1) % progress_interval == 0:
            logging.info(f"[ANALYSIS] Permutation progress: processed {idx + 1}/{total_features} features")

    if progress_interval and total_features % progress_interval != 0:
        logging.info(f"[ANALYSIS] Permutation progress: processed {total_features}/{total_features} features")
...
def run_univariate_feature_analysis(X, operations, labels, config, verbose=1):
    ...
    # Permutation testing disabled to reduce runtime
    results['permutation_mean_diff'] = np.zeros(X_filtered.shape[1])
    results['permutation_p'] = np.ones(X_filtered.shape[1])
    ...
    rename_map = {
        ...
        'permutation_mean_diff': 'univ_permutation_mean_diff',
        'permutation_p': 'univ_permutation_p'
    }
    df.rename(columns=rename_map, inplace=True)
    ranking_cols = [
        ('anova_rank', 'univ_anova_f', False),
        ('mi_rank', 'univ_mutual_info', False),
        ('mw_rank', 'univ_mann_whitney_u', False),
        ('bm_rank', 'univ_brunner_munzel_stat', False),
        ('perm_rank', 'univ_permutation_mean_diff', False),
    ]
    ...
    summary = {
        ...
        'top_features': {
            'anova': df.nsmallest(config.top_k, 'anova_rank')[['feature_name', 'univ_anova_f']].to_dict(orient='records'),
            'mutual_info': df.nsmallest(config.top_k, 'mi_rank')[['feature_name', 'univ_mutual_info']].to_dict(orient='records'),
            'mann_whitney': df.nsmallest(config.top_k, 'mw_rank')[['feature_name', 'univ_mann_whitney_u']].to_dict(orient='records'),
            'brunner_munzel': df.nsmallest(config.top_k, 'bm_rank')[['feature_name', 'univ_brunner_munzel_stat']].to_dict(orient='records'),
            'permutation': df.nsmallest(config.top_k, 'perm_rank')[['feature_name', 'univ_permutation_mean_diff']].to_dict(orient='records'),
        }
    }
...
def decide_univariate_method(summary_stats):
    ...
    if normal_fail < 30 and variance_fail < 30:
        ...
        rationale = ("Most features pass normality (~{:.1f}% fail) and variance equality (~{:.1f}% fail); "
                     "ANOVA/t-style scores are appropriate.").format(normal_fail, variance_fail)
        ...
        rationale = ("Normality fails for many features (~{:.1f}% fail) but variances remain comparable "
                     "(~{:.1f}% fail); Mann–Whitney U is safer.").format(normal_fail, variance_fail)
    else:
        decision = 'permutation_mean_diff'
        rationale = ("Both normality (~{:.1f}% fail) and variance equality (~{:.1f}% fail) are often violated; "
                     "use permutation-based ranking.").format(normal_fail, variance_fail)
...
def create_topk_figure(df, top_k, output_path):
    ...
    for idx, (metric, title) in enumerate(METRICS):
        ax = axes[idx]
        subset = (
            df.nlargest(top_k, metric)
              [['feature_name', metric]]
              .sort_values(metric)
        )
        ax.barh(subset['feature_name'], subset[metric], color='#1f77b4')
        ax.set_title(title)
        ax.set_xlabel('Score')
        ax.set_ylabel('')
...
def create_distribution_figure(df, output_path):
    rows, cols = 2, 3
    fig, axes = plt.subplots(rows, cols, figsize=(18, 10))
    axes = axes.flatten()
    ...
    for idx, (metric, title) in enumerate(METRICS):
        ...
        ax.hist(series, bins=40, color='#ff7f0e', alpha=0.85)
...
def create_intersection_figure(df, top_k, output_path):
    top_sets = {
        title: set(df.nlargest(top_k, metric)['feature_name'])
        for metric, title in METRICS
    }

    labels = [title for _, title in METRICS]
    size = len(labels)
...
def print_univariate_summary(df, top_k):
    ...
    for metric, title in METRICS:
        subset = df.nlargest(top_k, metric)[['feature_name', metric]]
        logging.info(f"\n{title}\n{'-' * len(title)}\n{subset.to_string(index=False, header=['Feature', 'Score'])}")
...
def main():
    univariate_config = SimpleNamespace(
        ...
        n_permutations=DEFAULT_UNIVARIATE_N_PERMUTATIONS,
        random_state=DEFAULT_UNIVARIATE_RANDOM_STATE,
        top_k=DEFAULT_UNIVARIATE_TOP_K,
        permutation_progress_interval=DEFAULT_UNIVARIATE_PERM_PROGRESS
    )
    ...
    create_distribution_figure(univariate_df, dist_fig)
