...
class FeatureExtractor:
    @staticmethod
    def extract_band_power_per_channel(epochs, freq_bands):
        """
    ...
    @staticmethod
    def extract_psd_per_channel(epochs, freq_bands):
        """
        Extracts PSD features from each channel of the given MNE epochs without averaging across channels or frequencies.
        ...
        for band, (low, high) in freq_bands.items():
            ...
            # Extract raw PSD values for each frequency in the band for each epoch and channel and store it for each band in the dictionary
