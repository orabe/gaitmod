...
class BaseModel(ABC):
    ...
    def evaluate(self, y_true, y_pred):
        ...
        if self.metrics.get('precision') is not None:
            self.metrics['precision'].append(precision_score(y_true, y_pred, average='weighted'))
        if self.metrics.get('recall') is not None:
            self.metrics['recall'].append(recall_score(y_true, y_pred, average='weighted'))
        if self.metrics.get('f1') is not None:
            self.metrics['f1'].append(f1_score(y_true, y_pred, average='weighted'))
        if self.metrics.get('confusion_matrices') is not None:
            self.metrics['confusion_matrices'].append(confusion_matrix(y_true, y_pred))
